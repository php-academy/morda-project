...
# Функция поднятия авторизации пользователя по Куке:
...
# Функция записи в сессию url текущей страницы для рефреша после авторизации:
# - если страница засечена, то удаляем сессию и зписываем новую.
# - в противном случае, сетим страницу.
function setThisPage() {
    if ( isset($_SESSION['refreshPage']) ) {
        unset($_SESSION['refreshPage']);
        $_SESSION['refreshPage'] = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
    } else $_SESSION['refreshPage'] = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
    return $_SESSION['refreshPage']; 
}
